...
Но один конфиг-фактор реально ест скорость: по умолчанию стоит **`--enforce-eager`**
(стабильный старт на Blackwell + меньше VRAM), который выключает CUDA-графы. В eager
каждый мелкий kernel запускается Python'ом отдельно, и при batch=1 GPU заметную долю
времени ЖДЁТ CPU — это и выглядит как 35–40% утилизации. Включение CUDA-графов
убирает накладные расходы запуска (десятки процентов к токенам/с на MoE):

```bash
# wsl/.env  (ВАЖНО: графам нужен запас VRAM — освободите его, снизив util)
JARVIS_QWEN_ENFORCE_EAGER=
JARVIS_QWEN_GPU_UTIL=0.80
```
затем `python jarvis.py up --profile <ваш>`. Если vLLM не стартует/падает на
захвате графов (sm_120 — свежая архитектура) — верните `--enforce-eager` обратно.
