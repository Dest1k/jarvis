...
## 5.2 Автономный агент-установщик (`install_agent.py`)

Полностью **самостоятельный** режим: локальная LLM (LM Studio) сама ведёт
развёртывание — анализирует состояние хоста, принимает решения, исполняет
команды, диагностирует ошибки и восстанавливается, **без вмешательства**.
