...
|---------|-------------------|
| **Мозги не стартуют**, `Engine core initialization failed` | Настоящая причина печатается в логе vLLM **до** финального traceback. `python jarvis.py diag` (тянет 150 строк). Частые причины: неполная докачка весов; нехватка VRAM (закройте GPU-приложения, снизьте `JARVIS_QWEN_GPU_UTIL`/`MAX_LEN`); несовместимая версия vLLM с архитектурой модели |
