...
def _setup_console_utf8() -> None:
    """
    UTF-8 для консоли Windows, иначе кириллица превращается в «ромбики со знаком
    вопроса» (консоль по умолчанию cp866/cp1251). Вызывать ДО basicConfig.
    """
...
def write_compose_env(profile: DeploymentProfile, qwen_model: str, uitars_model: str,
    ...
    # как отдельный модуль зрения). Если установку явно запускают в СОЛО-режиме
    # (JARVIS_ENABLE_UITARS=0 — единый мультимодальный мозг видит экран сам),
    # сразу пишем согласованные ключи: сервис UI-TARS выключен (COMPOSE_PROFILES
    # пуст), а «зрение» маршрутизируется на диспетчер. Обычно соло включают
    # профилем gemma4-solo уже после установки. См. wsl/profiles.json.
