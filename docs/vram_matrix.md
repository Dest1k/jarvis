...
### `gemma4-solo` (РЕКОМЕНДУЕТСЯ) — единый мозг, без UI-TARS

Один мультимодальный мозг **Gemma-4-26B-A4B** делает ВСЁ: планирует, пишет код И
**сам видит экран** (инструмент `gui` идёт в тот же инстанс — Gemma получает
скриншот и возвращает действие). Отдельный UI-TARS не нужен и **выключен**
(`JARVIS_ENABLE_UITARS=0`, `COMPOSE_PROFILES=` пусто → сервис `vllm-ui-tars` не
поднимается, но остаётся виден в Пульте). `gui` маршрутизируется на диспетчер:
`JARVIS_VISION_URL=http://vllm-qwen-coder:8001/v1`, `JARVIS_VISION_MODEL=qwen-coder`.
