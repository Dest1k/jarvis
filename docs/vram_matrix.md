...
Совпадает с матрицей выше: Qwen2.5-Coder-14B-AWQ (0.45) + UI-TARS-2B (0.20) +
аудио ≈ **22.8**, резерв ≈ **9.2**.

### `gemma4-tars15` (умный, под Blackwell)

| Сервис | Модель | Квант | Веса | KV (fp8) | util | Итого |
|--------|--------|-------|------|----------|:----:|:-----:|
| Диспетчер | **Gemma-4-26B-A4B MoE** (`nvidia/Gemma-4-26B-A4B-NVFP4`) | **NVFP4** (4-бит FP) | **~16.5** | ~2.0 @16k | `0.60` | **~19** |
| GUI/ОС | **UI-TARS-1.5-7B** (`flin775/UI-TARS-1.5-7B-AWQ`) | Int4 AWQ | ~5.2 | ~0.6 @8k | `0.20` | **~6.4** |
| Аудио | Whisper-v3 + Kokoro | int8/fp16 | — | — | — | **~2.0** |
| **Занято** | | | | | | **~27** |
| **Резерв** | | | | | | **~5** |

> **MoE 26B-A4B, а не dense-31B.** Реальный репозиторий `nvidia/Gemma-4-31B-IT-NVFP4`
> весит **~30 ГБ** (часть тензоров в bf16/fp8 — эмбеддинги Gemma на словарь 256k и
> т.п.), поэтому dense-31B **не помещается** рядом с UI-TARS+аудио. Берём **MoE
> 26B-A4B в NVFP4** (49→16.5 ГБ): всего 26B, **активно 4B/токен** — и влезает, и
> быстрее. Подтверждено сообществом (recipes.vllm.ai, обсуждения на HF).

**Низкие кванты «без фанатизма»:**
- **NVFP4** — родной для Blackwell 4-битный *float* (NVIDIA Model Optimizer),
  качество ≈ bf16. Для Gemma это важно: **AWQ роняет Gemma-3/4** (бенч 85.6→69.2%),
  а NVFP4/GPTQ-W4A16 держат ~bf16. Поэтому Gemma — в NVFP4 (или GPTQ), **не AWQ**.
- **KV-кэш fp8** (e4m3) — половинит память KV при ~нулевой потере. Это разумный
  предел; int4-KV уже «делает модель ребёнком» — не идём ниже.
- Веса 4-бит — нижняя разумная граница; int3/int2 не берём.

**Требования/тюнинг:** vLLM ≥ 0.17 (cu128) для NVFP4 на Blackwell (`vllm/vllm-openai:latest`
подходит). Резерв ~5 ГБ тоньше классики — не держите тяжёлые GPU-приложения;
при OOM: `JARVIS_QWEN_GPU_UTIL=0.55`, `JARVIS_QWEN_MAX_LEN=12288`.

**Ещё легче:** **Gemma-4-12B** (NVFP4/GGUF ~8-10 ГБ) — больше запас VRAM, всё ещё
сильная instruct-модель. Впишите её repo в Пульт → «🧠 Мозг-диспетчер» → «Своя
модель». Альтернативные NVFP4-репо MoE: `RedHatAI/gemma-4-26B-A4B-it-NVFP4`.
