...
|--------|--------|-------|------|----------|:----:|:-----:|
| Диспетчер | **Gemma-4-31B-it** (`nvidia/Gemma-4-31B-IT-NVFP4`) | **NVFP4** (4-бит FP) | ~16.5 | ~2.0 @16k | `0.60` | **~19.2** |
...
подходит). Резерв ~4.4 ГБ тоньше классики — не держите тяжёлые GPU-приложения;
при OOM: `JARVIS_QWEN_GPU_UTIL=0.55`, `JARVIS_QWEN_MAX_LEN=12288`.

**Альтернатива — MoE:** **Gemma-4-26B-A4B** (26B total, **4B активных**) — быстрее
и больше запас (4-бит веса ~13 ГБ). Если есть vLLM-совместимый квант (NVFP4/GPTQ) —
впишите repo в Пульт → «🧠 Мозг-диспетчер» → «Своя модель».
