...
Важно: эти unit-тесты проверяют детерминированный ReAct/tool loop. Поэтому перед
импортом orchestrator мы выключаем автономную декомпозицию и background runtime.
Полноценная автономия тестируется отдельными smoke/runtime проверками.
"""
...
# Эти тесты используют короткий scripted fake planner. Дополнительный LLM-вызов
# автономной декомпозиции съест первый fake-ответ и сделает проверки недетерминированными.
os.environ["JARVIS_AUTONOMOUS"] = "0"
os.environ["JARVIS_BACKGROUND_RUNTIME"] = "0"
...
# --------------------------------------------------------------------------- #
# Тесты разбора действий GUI/Vision
# --------------------------------------------------------------------------- #
async def test_parse_uitars_actions():
    ...
    assert p("") is None
    return "разбор GUI-действий (форматы + box-токены + мусор)"
...
async def test_linux_send_keys_routing():
    ...
    ex._run = fake_run
    await ex.send_keys("ctrl+s")
    assert captured[-1][-1] == "ctrl+s", captured[-1]
    await ex.send_keys("^s")
    assert captured[-1][-1] == "ctrl+s", captured[-1]
    await ex.send_keys("{ENTER}")
    assert captured[-1][-1] == "Return", captured[-1]
    return "Linux send_keys: 'ctrl+s' и '^s'/'{ENTER}'"


async def test_open_app_nonblocking():
    """open_app должен запускать детачем без захвата pipe."""
...
async def test_windows_screenshot_png_dims():
    ...
    tf.write(_b64.b64decode(_PNG_1x1))
    tf.close()
    try:
