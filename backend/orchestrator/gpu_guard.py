# -*- coding: utf-8 -*-
"""
gpu_guard.py — NVML thermal/VRAM guard for JARVIS runtime.

The guard is intentionally conservative: it always prefers throttling internal
workload density over changing global GPU power settings. Power-limit changes are
available only behind explicit JARVIS_ALLOW_GPU_POWER_LIMIT=1 and still return a
remediation proposal for the host bridge/HITL path.
"""
...
class GpuGuard:
    ...
    def _sample_nvml(self) -> GpuSnapshot:
        try:
            ...
            return GpuSnapshot(True, time.time(), "pynvml", name, int(util.gpu), mem_used, mem_total, mem_pct, int(temp), None)
        except Exception as exc:  # noqa: BLE001
    ...
    async def _sample_host_nvidia_smi(self) -> GpuSnapshot:
        ...
        res = await self.host_exec(query)
        ...
        try:
            ...
            return GpuSnapshot(True, time.time(), "nvidia-smi", name, util, used, total, round(used / total * 100, 1), temp, None)
        except Exception as exc:  # noqa: BLE001
    ...
    def _derive_throttle(self, snap: GpuSnapshot) -> int:
        ...
        if snap.mem_pct is not None:
            if snap.mem_pct >= self.crit_mem_pct:
                level = max(level, 3); reasons.append("critical VRAM pressure")
            elif snap.mem_pct >= self.warn_mem_pct:
                level = max(level, 2); reasons.append("high VRAM pressure")
        temp = snap.temp_mem_c or snap.temp_gpu_c
        if temp is not None:
            if temp >= self.crit_temp_c:
                level = max(level, 3); reasons.append("critical thermal pressure")
            elif temp >= self.warn_temp_c:
                level = max(level, 1); reasons.append("thermal warning")
    ...
    async def remediation_plan(self) -> dict[str, Any]:
        ...
        if self._throttle_level >= 1:
            actions.append({"kind": "throttle", "detail": f"insert inference delay {self.throttle_delay()}s"})
        if self._throttle_level >= 2:
            actions.append({"kind": "context_prune", "detail": "ask agent runtime to flush inactive session summaries"})
        if self._throttle_level >= 3:
            actions.append({"kind": "cycle_optional", "detail": "stop audio-layer/UI-TARS if profile marks them non-essential"})
        if os.environ.get("JARVIS_ALLOW_GPU_POWER_LIMIT") == "1" and self._throttle_level >= 3:
            limit = os.environ.get("JARVIS_GPU_SAFE_POWER_LIMIT_W", "420")
            actions.append({"kind": "host_command_requires_human", "detail": f"nvidia-smi -pl {limit}"})
        return {"snapshot": asdict(snap), "actions": actions}
