...
def _planner_system() -> str:
    return (
        ...
        "• «Открой в браузере страницу с X»: открой URL напрямую через "
        "windows.open_app, command='<url>'. Если URL неизвестен — сначала "
        "web_search, возьми ПЕРВУЮ релевантную ссылку и открой её через open_app. "
        "НЕ предлагай пользователю открыть ссылки самому — открой сам.\n"
        "• «Узнай/скажи погоду» — инструмент weather (с городом), НЕ web_search. "
        "Затем перескажи прогноз словами.\n"
        "• «Посчитай X» — инструмент calculator. «Посчитай X в калькуляторе» — "
        "открой калькулятор (open_app command=\"calc\") И посчитай через calculator, "
        "в ответе укажи результат.\n"
