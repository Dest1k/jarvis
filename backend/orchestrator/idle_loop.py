# -*- coding: utf-8 -*-
"""
idle_loop.py — Background Idle Thinking / diagnostics / self-healing loop.

The loop runs only when no chat turn is active and the GPU guard reports safe
pressure. It is conservative by default: diagnostics run automatically, code
modification requires JARVIS_SELF_HEAL_ENABLE=1 and merges always require human
approval through HUD/HITL.
"""
...
class BackgroundIdleLoop:
    ...
    async def _self_heal(self, anomaly: str) -> None:
        if not self.self_heal_enabled or self.host_exec is None:
            await self._emit({
                "level": "info",
                "message": "Self-heal is in diagnostic-only mode. Set JARVIS_SELF_HEAL_ENABLE=1 to allow branch/test cycles.",
            })
            return
        ...
        msg = (
            f"Sir, I detected an anomaly and prepared branch {branch}. Tests passed. Shall I open/merge the pull request?"
            if ok else f"Sir, I detected an anomaly, but the autonomous validation failed on branch {branch}."
        )
