...
Инструменты — это то, ЧЕМ оркестратор (Qwen) реально действует в мире:
    ...
    shell         — shell-команды на Linux-стороне (администрирование/быт) в
                    изолированном окружении с полным набором утилит.
    ...
    gui           — визуальное управление Windows: скриншот хоста передаётся
                    «зрению» системы (в СОЛО-режиме это сам мозг Gemma-4, в
                    классических профилях — UI-TARS), оно возвращает одно
                    действие (клик/ввод/прокрутка), мост его исполняет.
...
# --- 1b. Shell на Linux-стороне (администрирование/быт) -------------------- #
async def tool_shell(args: dict[str, Any], ctx: ToolContext) -> dict[str, Any]:
    """
    Выполнить shell-команду на Linux-стороне (в изолированном sandbox-контейнере
    с полным набором утилит: bash, coreutils, gcc/g++, python3, node, git).

    Это «правая рука» JARVIS как Linux-администратора для безопасных операций:
    обработка текста/файлов, скрипты, пакеты, диагностика. Рабочий каталог
    /workspace переживает вызовы в рамках сессии — можно строить состояние по
    шагам. Среда изолирована: команды НЕ трогают реальный хост.

    Для управления реальным Linux/WSL-хостом используйте инструмент windows с
    action=exec и командой вида `wsl bash -lc "..."` (через RPC-мост).
    """
    command = str(args.get("command") or args.get("cmd") or "").strip()
    if not command:
        return {"ok": False, "content": "Не задан 'command'."}
    workdir = str(args.get("workdir") or "/workspace").strip() or "/workspace"
    timeout = int(args.get("timeout", 60))
    stdin = str(args.get("stdin", ""))

    cmd_b64 = base64.b64encode(command.encode("utf-8")).decode("ascii")
    stdin_b64 = base64.b64encode(stdin.encode("utf-8")).decode("ascii")
    # Команду и stdin передаём base64 → не страдаем от кавычек/переводов строк.
    inner = (
        f"mkdir -p {workdir} 2>/dev/null; cd {workdir} 2>/dev/null || cd /; "
        f"echo {stdin_b64} | base64 -d > /tmp/.shin; "
        f"echo {cmd_b64} | base64 -d > /tmp/.shcmd; "
        f"timeout {timeout} bash -l /tmp/.shcmd < /tmp/.shin 2>&1; "
    )
    from . import dockerapi
    try:
        rc, output = await dockerapi.exec_run(
            SANDBOX_CONTAINER, ["bash", "-lc", inner], timeout=timeout + 5)
    except Exception as exc:  # noqa: BLE001
        return {"ok": False,
                "content": f"Не удалось выполнить команду в sandbox (Docker API): {exc}"}
    body = (output or "").rstrip()
    if not body:
        body = "(команда выполнена, вывод пуст)"
    tail = f"\n[код возврата: {rc}]"
    return {"ok": rc == 0, "content": _truncate(body + tail),
            "data": {"returncode": rc, "output": output, "command": command}}
...
# --- 3. Визуальное управление GUI (зрение мозга / UI-TARS) ---------------- #
async def tool_gui(args: dict[str, Any], ctx: ToolContext) -> dict[str, Any]:
    ...
    «зрению» системы вместе с целью, получить ОДНО действие и выполнить его
    через мост.

    Зрение — это llm.VISION_URL/VISION_MODEL: в СОЛО-режиме это сам мозг Gemma-4
    (мультимодальный — он же планирует, он же смотрит на экран), в классических
    профилях — отдельный UI-TARS. Грамматика действий одинакова для обоих, что
    делает инструмент модель-независимым.

    Это лучшее-усилие: точное GUI-управление зависит от модели/железа, поэтому
    действия атомарные, а оркестратор вызывает gui повторно по шагам.
    """
    if ctx.bridge is None:
    ...
    system = (
        "You are the GUI-control vision module of JARVIS, operating a Windows "
        "computer through screenshots. Given the screenshot and the task, decide "
        "the NEXT SINGLE atomic action to make progress.\n\n"
        "## Action Space (coordinates are NORMALIZED 0-1000 of the screenshot)\n"
        "click(start_box='(x,y)')\n"
        ...
        "finished(content='done')    # the task is already complete\n\n"
        "Rules: pick exactly ONE action; aim coordinates at the CENTER of the "
        "target control; if the goal is clearly already satisfied, output "
        "finished(). Output EXACTLY two lines and nothing else:\n"
    ...
    try:
        raw = await llm.chat(messages, base_url=llm.VISION_URL, model=llm.VISION_MODEL,
                             temperature=0.0, max_tokens=320, timeout=120)
    except Exception as exc:  # noqa: BLE001
        return {"ok": False, "content": f"Модуль зрения недоступен: {exc}"}

    act = _parse_gui_action(raw)
    if not act:
        return {"ok": False, "content": f"Зрение вернуло неразборчивый ответ: {raw[:300]}"}
    kind = act["kind"]

    if kind in ("finished", "done", "complete"):
        return {"ok": True, "content": f"GUI: цель достигнута. {act.get('content', '')}"}
    if kind == "wait":
        return {"ok": True, "content": "GUI: ожидание (экран ещё меняется)."}
    if kind in ("fail", "impossible"):
        return {"ok": False, "content": f"GUI: не удалось — {act.get('content', '')}"}
    ...
    if kind in ("click", "left_single", "left_double", "right_single",
        ...
        return {"ok": False, "content": f"Неизвестное GUI-действие: {kind}"}
...
def _parse_gui_action(text: str) -> Optional[dict[str, Any]]:
    """Разобрать GUI-действие из строки 'Action: click(start_box='(x,y)')'.

    Грамматика — общая для модуля зрения (Gemma-4 в соло) и UI-TARS, поэтому
    разбор одинаков для обоих «мозгов».
    """
    if not text:
        return None
...
# --- 13b. Перевод текста (без ключей) -------------------------------------- #
async def tool_translate(args: dict[str, Any], ctx: ToolContext) -> dict[str, Any]:
    """Перевести текст на нужный язык (Google translate gtx-эндпоинт, без ключа)."""
    text = str(args.get("text") or args.get("query") or "").strip()
    if not text:
        return {"ok": False, "content": "Не задан 'text'."}
    target = str(args.get("to") or args.get("target") or "ru").strip() or "ru"
    source = str(args.get("from") or args.get("source") or "auto").strip() or "auto"
    try:
        async with httpx.AsyncClient(timeout=20, follow_redirects=True,
                                     headers={"User-Agent": HTTP_UA}) as cli:
            r = await cli.get("https://translate.googleapis.com/translate_a/single",
                              params={"client": "gtx", "sl": source, "tl": target,
                                      "dt": "t", "q": text})
            r.raise_for_status()
            data = r.json()
    except Exception as exc:  # noqa: BLE001
        return {"ok": False, "content": f"Ошибка перевода: {exc}"}
    try:
        translated = "".join(seg[0] for seg in data[0] if seg and seg[0])
        detected = data[2] if len(data) > 2 else source
    except Exception:  # noqa: BLE001
        return {"ok": False, "content": "Не удалось разобрать ответ переводчика."}
    return {"ok": True, "content":
            _truncate(f"Перевод ({detected}→{target}):\n{translated}")}
...
class ToolRegistry:
    ...
    def _register_defaults(self) -> None:
        ...
        self.add(Tool(
            "shell",
            "Выполнить shell-команду на LINUX-стороне в изолированном окружении "
            "(bash, coreutils, python3, node, gcc/g++, git). Для администрирования "
            "Linux и бытовых задач: обработка файлов/текста, скрипты, диагностика, "
            "пакеты. Каталог /workspace переживает вызовы в сессии. Реальный хост "
            "не затрагивается (изоляция). Для команд на РЕАЛЬНОМ Linux/WSL-хосте — "
            "windows.exec с командой 'wsl bash -lc \"...\"'.",
            {"command": "shell-команда (можно конвейеры, многострочно)",
             "workdir": "(опц.) рабочий каталог, по умолч. /workspace",
             "stdin": "(опц.) ввод на stdin",
             "timeout": "(опц.) лимит секунд, по умолч. 60"},
            tool_shell,
        ))
        self.add(Tool(
            "gui",
            "Визуально управлять Windows СВОИМИ ГЛАЗАМИ: ты получаешь скриншот "
            "экрана и делаешь один шаг (клик/двойной клик/правый клик/ввод "
            "текста/горячие клавиши/прокрутка). Использовать, когда нет "
            "CLI-способа, только графический (нажать кнопку, выбрать пункт меню, "
            "элемент окна). Вызывать повторно по шагам (каждый раз свежий "
            "скриншот) до достижения цели.",
        ...
        self.add(Tool(
            "translate",
            "Перевести текст на нужный язык (по умолч. на русский). Определяет "
            "исходный язык автоматически.",
            {"text": "текст для перевода",
             "to": "(опц.) язык назначения: ru,en,de,... по умолч. ru",
             "from": "(опц.) исходный язык, по умолч. auto"},
            tool_translate,
        ))
        self.add(Tool(
            "system_info",
