...
class ToolRegistry:
    ...
    def _register_defaults(self) -> None:
        ...
        self.add(Tool(
            "windows",
            "ГЛАВНЫЙ инструмент системного администратора: полный доступ к "
            "ХОСТ-машине через защищённый мост (работает и на Windows, и на Linux — "
            "мост сам выбирает бэкенд). Любая команда ОС (exec=cmd/bash, "
            "powershell=pwsh): службы, процессы, реестр, диски, сеть, установка "
            "программ (winget), журналы, планировщик. Плюс: запуск программ "
            "(open_app), ВВОД ТЕКСТА в активное окно (paste_text — надёжно, через "
            "буфер обмена), управляющие клавиши (send_keys), файлы "
            "(read_file/write_file), скриншот, медиа/громкость (media_hook), "
            "процессы (kill_process), питание (system_power). Чтобы 'написать "
            "текст/код в программе' — open_app, затем paste_text. Деструктивное — "
        ...
        self.add(Tool(
            "gui",
            "ВИЗУАЛЬНО выполнить задачу на экране: твои глаза + мышь + клавиатура "
            "(GUI-суб-агент; в монолите экраном управляет сам мозг). ОДИН вызов "
            "доводит подзадачу до конца сам: внутри цикл «скриншот→клик/ввод/"
            "прокрутка→скриншот». Используй, когда нет CLI-способа ИЛИ команда/"
            "скрипт не сработали — нажать кнопку, пройти мастер установки, "
            "переключить настройку в окне, выбрать пункт меню. Формулируй ЦЕЛЬ "
            "человеческим языком, не отдельные клики.",
