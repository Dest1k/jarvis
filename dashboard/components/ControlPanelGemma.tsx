...
export default function ControlPanelGemma() {
  ...
  const refresh = useCallback(async () => {
    try { const ov = await getJson(`${API}/overview`); setOverview(ov); setCfg(ov.config || ""); } catch { setOverview(null); }
  ...
  const save = async () => { setBusy("config"); await postJson(`${API}/config`, { content: cfg }); setBusy(""); };
  ...
  return <div className="control-v3">
    ...
    <div className="panel"><h3>Конфигурация</h3><textarea value={cfg} onChange={(e) => setCfg(e.target.value)} rows={12} style={{ width: "100%" }} /><p><button className="btn primary" disabled={!!busy} onClick={save}>Сохранить</button></p></div>
  </div>;
}
